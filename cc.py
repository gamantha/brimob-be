...
def get_tracker_activeuser_all():
    ...
    db2 = get_db2()
    cursor = db2.cursor(dictionary=True)
    query = "select tracker_user.id, tracker_user.iduser, user.username, user_data.nama, user_data.pangkat, region.id, region.region_name, department.department_name, position.position_name, lat, lon, altitude, tracker_user.timestamp " \
            "from tracker_user LEFT JOIN user on tracker_user.iduser = user.iduser LEFT JOIN position on user.position_id = position.id " \
            "LEFT JOIN user_data on user_data.iduser = tracker_user.iduser LEFT JOIN department on department.id = position.department_id LEFT JOIN region on region.id = department.region_id " \
...
def get_tracker_activeuser_byregion():
    ...
    query = "select tracker_user.id, tracker_user.iduser, user.username, user_data.nama, user_data.pangkat, region.region_name, department.department_name, position.position_name, lat, lon, altitude, tracker_user.timestamp " \
            "from tracker_user LEFT JOIN user on tracker_user.iduser = user.iduser LEFT JOIN position on user.position_id = position.id " \
            "LEFT JOIN user_data on user_data.iduser = tracker_user.iduser LEFT JOIN department on department.id = position.department_id LEFT JOIN region on region.id = department.region_id " \
            "where tracker_user.id in (select max(tracker_user.id) from tracker_user group by iduser) AND region_id = %s AND `timestamp` > DATE_SUB(now(),INTERVAL 12 HOUR)"
...
@cc_blueprint.route('/get_tracker_activeuser_byoperation', methods=["POST"])
...
def get_tracker_devices():
    ...
    query = "select a.loc_id, a.tracker_device_id, a.lat, a.lon, a.altitude, a.speed, a.hdop, tracker_device.device_name, tracker_device.device_type, tracker_video.video_url, tracker_device.`status` from " \
            ...
            "LEFT JOIN tracker_device on a.tracker_device_id = tracker_device.device_id left join tracker_video on tracker_video.tracker_device_id = tracker_device.device_id"
...
@cc_blueprint.route('/get_tracker_ranmor_byregion', methods=["POST"])
...
@cc_blueprint.route('/get_giatharian_image_ios', methods=["GET"])
def get_giatharian_image_ios():
    res = dict()
...
@cc_blueprint.route('/get_giatinsidentil_image_ios', methods=["GET"])
def get_giatinsidentil_image_ios():
    res = dict()
    # filename = request.form['filename']
    filename = request.args.get("filename", None)
    # name = user_id + ".jpeg"
...
def laporan_giat_user():
    ...
    query = "SELECT laporan_giat.id, laporan_giat.user_id, user.username, laporan_giat.region_id, laporan_giat.department_id, laporan_giat.no_laporan, laporan_giat.tgl_laporan, " \
            "laporan_giat.laporan_text, laporan_giat.lat_pelapor, laporan_giat.long_pelapor, laporan_giat.alamat, " \
            "laporan_giat.laporan_subcategory_id, subkategori.sub_kategori,  laporan_giat.image_file, " \
            "DATE_FORMAT(laporan_giat.tgl_submitted, '%Y-%m-%d %H:%i:%S') as tgl_submitted FROM laporan_giat " \
...
def laporan_giat_list_harian():
    ...
    query = "SELECT laporan_giat.id, laporan_giat.user_id, user.username,  user_data.nama, user_data.pangkat, laporan_giat.region_id, region.region_name, laporan_giat.department_id, department.department_name, no_laporan, " \
            "tgl_laporan, DATE_FORMAT(tgl_laporan, '%d/%m/%Y') as tgl_for_search, lat_pelapor, long_pelapor, laporan_giat.alamat, " \
            "laporan_text, laporan_subcategory_id, subkategori.sub_kategori, image_file, DATE_FORMAT(laporan_giat.tgl_submitted, '%Y-%m-%d %H:%i:%S') as tgl_submitted FROM laporan_giat " \
            "LEFT JOIN region ON region.id = laporan_giat.region_id " \
            "LEFT JOIN subkategori ON subkategori.idsubkategori = laporan_giat.laporan_subcategory_id " \
            "LEFT JOIN department ON department.id = laporan_giat.department_id " \
            "LEFT JOIN user_data on user_data.iduser = laporan_giat.user_id " \
...
def laporan_giat_list_peruser():
    ...
    query = "SELECT laporan_giat.id, laporan_giat.user_id, user.username, user_data.nama, laporan_giat.region_id, " \
            ...
            "LEFT JOIN user ON user.iduser = laporan_giat.user_id " \
            "LEFT JOIN user_data ON user_data.iduser = laporan_giat.user_id " \
            "LEFT JOIN department ON department.id = laporan_giat.department_id WHERE laporan_giat.user_id = %s AND laporan_giat.laporan_subcategory_id = %s ORDER BY laporan_giat.id DESC"
    cursor.execute(query, (userid,laporan_subcategory_id,))
    record = cursor.fetchall()
    cursor.close()
    result = dict()
